...
import { getBotTranscript, getBotRecording } from '@imisi/bots/recall/client'
  ...
  async ({ event, step }) => {
    ...
    const segments = await step.run('store-transcript', async () => {
      const formatted = recallTranscript.map((seg: any) => ({
        ...
        start_ms: seg.words[0]?.start_timestamp * 1000 ?? 0,
        end_ms: seg.words[seg.words.length - 1]?.end_timestamp * 1000 ?? 0,
      }))
